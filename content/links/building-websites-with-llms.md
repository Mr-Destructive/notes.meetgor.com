---
title: "Building Websites with LLMs"
date: 2026-01-24
draft: false
---

# Building Websites with LLMs

**Link:** https://blog.jim-nielsen.com/2025/lots-of-little-html-pages

## Context

[Building Websites with LLMs](https://blog.jim-nielsen.com/2025/lots-of-little-html-pages): This was a really a nice post, need to add this to my blog as well. I want to write more and the more I write I will create more sections like TILs, Thoughts, Link Blog, Articles, Series, Tags, and we can go on, I want to keep my existing site simple yet, I want to organise them in a susinct way. ## Wrote

**Source:** techstructive-weekly-33
